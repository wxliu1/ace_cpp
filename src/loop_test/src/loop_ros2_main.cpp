...
LoopClosure::LoopClosure(const ros_nh pnh, const std::string &cfg_path)
{
    ros_io = std::make_shared<ROS_IO>(pnh, cfg_path);
    std::string pkg_path = ros_io->cfg_path_;
    // 获取support_files
    {
...
void LoopClosure::ros_topic_receive_init()
{
    ros_io->add_image_ = std::bind(&LoopClosure::add_left_image, this, std::placeholders::_1);
    ros_io->add_point_ = std::bind(&LoopClosure::add_feature_point, this, std::placeholders::_1, std::placeholders::_2);
    ros_io->add_pose_ = std::bind(&LoopClosure::add_pose, this, std::placeholders::_1);
    ros_io->ros_topic_receive_init();
}
...
void LoopClosure::add_feature_point(point_type &point, ros_pointcloud2 &cloud_msg)
{
    m_buf.lock();
    if (point_buf.size() > buffer_size)
    {
        point_buf.pop();
    }
    ...
    for (auto &point : point.points_3d)
    {
        Eigen::Vector3d tmp = r_drift * (w_r_vio * Eigen::Vector3d(point.x, point.y, point.z) + w_t_vio) + t_drift;
        point.x = tmp.x();
        point.y = tmp.y();
        point.z = tmp.z();
    }
    ros_io->pub_loop_points(point, cloud_msg);
}
...
void LoopClosure::add_pose(pose_type &pose)
{
    ...
    ros_io->pub_loop_odom(pose.position, pose.orientation, pose.timestamp);
}
...
void LoopClosure::loadKeyFrame(KeyFrame *cur_kf)
{
    ...
    ros_io->addBasePath(cur_kf->time_stamp, P, Q);
...
void LoopClosure::addKeyFrame(KeyFrame *cur_kf, bool add_brief)
{
    ...
    ros_io->push_loop_path(P, Q, cur_kf->time_stamp, cur_kf->sequence);
    keyframelist.push_back(cur_kf);
    ros_io->pub_loop_path(sequence_cnt);
    m_keyframelist.unlock();
}
...
void LoopClosure::updatePath()
{
    ...
    for (it = keyframelist.begin(); it != keyframelist.end(); it++)
    {
        ...
        if ((*it)->sequence)
        {
            ros_io->push_loop_path(P, Q, (*it)->time_stamp, (*it)->sequence);
        }
    }
    ros_io->pub_loop_path(sequence_cnt);
    m_keyframelist.unlock();
...
void LoopClosure::command()
{
    while (loop_on_flag)
    {
        if (ros_io->save_keyframe)
        {
            ...
            m_process.unlock();
            ros_io->save_keyframe_done = true;
            ros_io->save_keyframe = false;
        }
...
int main(int argc, char **argv){
    ...
    auto node = rclcpp::Node::make_shared("loop", node_options);
    rclcpp::executors::MultiThreadedExecutor executor(rclcpp::ExecutorOptions(), 10);
    std::string cfg_path = "/home/lwx/S3_ws/src/loop_test/loop/";

    std::shared_ptr<LoopClosure> loop_algo;
    loop_algo = std::make_shared<LoopClosure>(node, cfg_path);
    loop_algo->Start();
